...
- 两年使用相同分析标准，分别结合丙午、丁未相对于此人日主的十神、原局月令与藏干、当年大运及实际存在的冲合刑害来解释。每年至少点明一个此人具体命盘依据；没有成立的关系不得编造，见合不能直接判合化，见冲不能直接判灾祸。
- 五行的生活化比喻不能代替十神判断，不能把火一律解释为曝光、土一律解释为稳定。程序未提供确定的旺衰和喜忌时，须结合已有资料谨慎判断，不得把模型推断冒充程序计算结果。
