...
with st.sidebar:
    ...
    is_live_talk_mode = st.toggle(
        ...
        help="只影响直播快速简评的表达结构，不改变排盘和命理计算。开启后会把结果组织成 2.5-3.5 分钟的六张口播卡片。",
    )
...
【直播目标】
- 把个人结果重新组织成观众也能听懂的通用内容。
- 先给一个五维度简要分析，再给六张主卡片；简要分析本身要完整，不能只用一句带过；主口播总时长控制在 3-4 分钟，中文总字数约 1200-1600 字。
- 语气像直播间自然讲解，不像书面报告；一句话独立成段，便于照着口播。
- 术语必须先解释，再套回这个人，不要连续讲 10 分钟。
- 任何疑问句之前必须先给结论，不能只提问不回答。
- 只要向观众提出选择题，就必须同时准备每个选项的对应解答，不能让主播现场没有下文。
- 不使用夸张承诺，比如“巨大财富翻盘”“一定暴富”“必有大灾”。
- 不新增未解释的专业判断，比如“岁运并临”“水火既济”“火中取金”；如果必须用术语，先用白话解释并给出落地结论。

【必须严格输出 1 个五维度简要分析 + 6 张主卡片 + 1 个互动备答区，标题照抄】

### 0. 简要分析（60-90秒）
这一部分不是一句总评，也不是提纲；必须按五个维度完整呈现，给主播可以直接开口的开场稿。
总字数建议 350-550 字，每个维度至少 2-4 句，必须包含结论、原因、表现/例子和一个落地建议，不能只写抽象形容。
每个维度都要回到这个人的命盘，不要写成泛泛的命理科普。
必须照抄以下五个小标题：
【基本命盘】
说明日主、季节/月令、五行整体结构，点出最核心的命局关键词。
如果程序预校验给出四柱，必须引用日主和最明显的五行旺衰；如果信息不足，要明确说是初步判断。
这一段必须落到“这个命盘最核心的矛盾是什么”，不能只说五行名字。
至少写出一个命盘结论和一个白话解释，例如“强在哪、弱在哪、卡在哪”。

【事业】
说明事业风格、适合的工作方式、最容易卡住的地方。
必须明确说出：她更适合冲、守、合伙、研究、管理还是输出型工作。
最后补一句现实建议，例如“先稳节奏再开新项目”或“先借团队再单打独斗”。
至少给出一个事业优势和一个事业卡点。

【财富】
说明财富机会来自哪里，财富风险在哪里，避免只说“有财/没财”。
必须说清楚是靠正财、偏财、项目、专业、资源还是人脉。
必须补一个风险点，例如“容易漏财”“容易赚得快花得也快”“适合慢慢累积”。
至少写出“怎么来钱”和“怎么守钱”。

【情感】
说明亲密关系里的优势和压力点，不要只说桃花多或感情差。
必须说清楚她在关系里更像付出型、表达型、主导型还是敏感型。
最后给出一个互动建议，例如“沟通要先讲感受，再讲对错”。
至少写出一个关系优势和一个相处难点。

【健康】
说明精力、压力、作息或五行偏性对应的注意点。
必须说的是倾向和压力，不下诊断。
涉及疾病时只说“注意倾向/压力”，必须提醒形而上学不替代医学诊断。
至少写出一个精力状态判断和一个生活提醒。

### 1. 通用知识点（20-30秒）
先讲一个所有观众都能学到的八字知识点。
例如某五行、某十神、某冲合、某旺衰代表什么。
不要一上来只说这个人的私事。

### 2. 个人核心结果（45-60秒）
用“以某某为例”切回这个人。
说清楚此人最明显的命理结构、性格/事业/感情/压力核心。
必须包含一个明确结论，但不要恐吓。

### 3. 转化成通用知识（20-30秒）
把刚才这个人的结果抽象成观众也能用的判断方法。
句式类似：“一般来说，某某结构的人，不是……而是……”

### 4. 个人后续重点（45-60秒）
继续回到此人，讲后面真正需要关注的重点。
优先回应用户诉求；如果没有诉求，就从事业、感情、家庭、健康能量里选最关键的一项。
这一张必须把本次测算的核心问题先回答完整，不能把答案留到后面。
结尾可以自然过渡，但不能用疑问句代替结论。

### 5. 留下一个内容悬念（10秒）
只留一个“下一段可以继续展开的方向”，不新增新的重大判断。
悬念必须建立在第 4 张已经回答过的结论上。
禁止写“想知道如何……吗？”这种只提问不回答的句式。
推荐句式：“下一段如果继续讲，我会重点拆她的事业节奏/家庭关系/感情避坑里最关键的一点。”

### 6. 提出真实问题（10-15秒）
先用一句话给出最终判断，再向直播间提出一个真实互动问题。
必须使用这个结构：“我的判断是……所以现在不是……而是……。你们想先听 A，还是 B？”
问题只用于选择下一段展开方向，不能替代本次结论。
不要问空泛问题，不要连续提出两个以上问题。
这里提出的 A/B 两个选项，必须和第 7 部分的两个备答标题完全对应。

### 7. 互动备答（观众选择后使用，不计入主卡片时长）
这一部分不是继续提问，而是给主播准备的两段可直接口播的后续解答。
必须严格输出两个小标题：
【如果观众选 A：……】
【如果观众选 B：……】
每个选项各 80-140 字，必须给出明确解答、原因和一个落地提醒。
两个备答都必须基于同一张命盘，不得为了制造悬念而编新结论。
不要再在备答结尾继续抛新的二选一问题。

【示范风格】
先简要分析：
【基本命盘】这类命盘的核心，不是“有没有能力”，而是“这个能量结构会不会把自己压得太满”。如果火土偏重，常见表现是行动快、责任感强、执行力不差，但也容易累、急、绷得太紧。
【事业】事业上更适合有目标、能落地的路径，不适合长期空转或反复内耗。优势是扛事和推进，卡点是容易把自己当成唯一发动机。
【财富】钱往往不是没有，而是来得快、花得也快，或者赚得到但守得辛苦。真正要补的是节奏、分配和长期累积。
【情感】关系里通常更愿意承担，也更容易把压力带进互动里。优点是负责，难点是压力大时表达会变硬。
【健康】重点看精力消耗和作息压力，而不是下医学结论。越忙越要留休息和恢复空间。

我的判断是，她现在最需要调整的，不是能力，而是压力的分配方式。

下一段如果继续讲，我会拆她事业节奏和家庭关系里面最关键的一个卡点。

你们想先听她的事业节奏，还是家庭关系？

### 7. 互动备答（观众选择后使用，不计入主卡片时长）

【如果观众选事业节奏：事业上她不是不能冲，而是不能一直靠硬扛来冲。她适合把目标拆小，把执行交给流程和团队，不要什么都自己压着做。接下来最重要的是减掉低回报的消耗，把精力放在真正能累积资源的位置。】

【如果观众选家庭关系：家庭关系里她最大的问题不是不在乎，而是压力太大以后表达会变硬。她需要先把自己的节奏稳住，再去沟通需求。越是火土旺的人，越要学会把承担说出来，而不是一边付出一边委屈。】
"""
            else:
