...
- 如果程序提示“时柱未知”或“仅预排年月日三柱”，必须在开头 3 段内说明：这是时辰未知的直播简评，只能看年月日三柱，不能断子女晚年、具体小时对应的神煞和高度细节。

【直播目标】
- 把个人结果重新组织成观众也能听懂的通用内容。
- 保持自然直播讲解节奏，不要切成六张卡片，不要输出“互动备答区”，不要写“第几张卡片”。
- 正文总时长控制在 2.5-3.5 分钟，中文总字数约 1000-1400 字。
- 语气像直播间自然讲解，不像书面报告；一句话独立成段，便于照着口播。
- 术语必须先解释，再套回这个人，不要连续讲 10 分钟。
- 任何疑问句之前必须先给结论，不能只提问不回答。
- 互动点要自然插在正文里，像主播顺口带观众参与，而不是单独另开板块。
- 可以有 2-4 个互动句，但每个互动句前后必须已经给出答案，不能把关键结论留给观众选择。
...
- 禁止把不同命盘都写成“负担太多、压力大、总是把责任放在自己身上扛”。
...
五个维度可以用【基本命盘】【事业】【财富】【情感】【健康】做轻量提示，也可以融入自然段，但不能漏掉。
...
【正文节奏要求】
1. 开头 2-3 段先给总判断：这个命盘最突出的主轴是什么，不要先套“压力/责任”。
2. 接着用一个通用知识点带入个人命盘：先解释术语，再说“放到这个人身上就是……”。
3. 然后依次讲基本命盘、事业、财富、情感、健康，每个维度至少 2-4 句，必须有明确结论、命理原因、现实表现和落地建议。
4. 如果用户填写了核心诉求，必须在正文中优先回答，不要放到最后悬着。
5. 结尾用 2-3 段收束：给出本次最重要建议，再带一个自然互动点。

【互动点写法】
互动句要像直播现场自然插话，例如：
“直播间如果你也是这种一忙起来就停不下来的人，可以打一个 1，我继续往下讲。”
“你们注意听，这种命盘不是没有机会，而是机会来了以后能不能接稳。”
“这一点很多人会误会，以为财旺就是一定轻松赚钱，其实真正要看日主能不能承载。”
“如果你们想听更细的事业节点，我后面可以继续拆，但这段先把结论讲清楚。”
...
【示范风格】
这位朋友的命盘，第一眼要先看主轴，不是每个人都按压力大来讲。

如果一个人食伤明显，我们讲的是表达、创意、流量和不喜欢被管。

如果一个人财星明显，我们讲的是资源、机会、钱从哪里来，以及能不能守住。

如果一个人印星明显，我们讲的是学习、贵人、保护力，也要提醒不要想太多而行动慢。

所以直播间听的时候，不要只听一句性格标签，要看这个盘到底是哪一种力量最突出。

【事业】比如食伤旺的人，事业上更适合表达、内容、销售、创意、教学、顾问类路线，越能输出越容易打开局面。

但食伤旺也有卡点，就是不喜欢被束缚，遇到规则太死的环境容易烦，所以建议把自由度和稳定现金流同时设计好。

【财富】这种盘的钱往往来自作品、口才、技能转化和副业项目，不一定只靠固定工资。

守财建议是先把收入分层，生活钱、投资钱和试错钱分开，不要一有灵感就全部投进去。

【情感】感情里优势是有趣、有表达、有吸引力，难点是情绪上来时容易说得太直。

建议先说感受，再说要求，不要把沟通变成辩论。

【健康】健康这里只讲倾向，不替代医学诊断。

如果火气或食伤偏明显，要留意熬夜、上火、情绪急躁和作息波动，先把睡眠稳住，运势才容易接得住。

我的判断是，这类人不是没机会，而是要把表达力变成稳定作品和稳定收入。

直播间如果你也是“想法很多、表达欲强，但不想被管太死”的类型，可以打一个 1，这种命盘要学会把才华落到节奏里。
"""
            else:
                live_constraint = "\n\n⚠️【重要提醒：直播快速简评模式】：当前由直播快速引擎驱动，你【只需输出】【### 📜 PARTE 0: 直播总体简单评价】这一个模块，严禁输出 PARTE I/II/III/IV 等任何后续深度模块。务必满足：纯中文、1000字以内、一句一段（段间空行）、直白通俗、明确包含生肖与纳音属性、精准解答核心具体诉求、自带转化钩子文案。必须依据【程序排盘预校验】的日主、年月日柱或四柱、五行旺衰和十神主轴来写。禁止把不同命盘都写成“负担太多、压力大、总是把责任放在自己身上扛”；只有命盘确实官杀重、土重、财官压身、火土过旺或诉求指向压力时，才可以这样讲。如果程序提示“时柱未知”或“仅预排年月日三柱”，必须说明这是时辰未知的直播简评，不能强断时柱、子女晚年和具体时柱神煞。"
        else:
